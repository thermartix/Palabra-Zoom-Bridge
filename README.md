...
# Use "palabra" to disable local speed adjustment and rely on [palabra] auto_tempo.
...
The `[palabra]` section is tuned to favor complete returned audio over the lowest possible latency. The current profile follows Palabra's recommended queue defaults for long sentences: `segment_confirmation_silence_threshold = 0.7`, `desired_queue_level_ms = 5000`, `max_queue_level_ms = 20000`, `auto_tempo = true`, and `max_tempo = 1.45`. The earlier known-good complete-audio profile used a stricter `0.8` silence threshold with `3000/8000` queue levels and Palabra `auto_tempo = false`; keep a local snapshot before changing these values during live tests. Change the segmentation booleans carefully: non-silence confirmation, sentence splitting, or partial translation can make Palabra split or replace speech segments unpredictably, which may bring missing words/audio chunks back.
...
To test the SDK audio path locally before Palabra is connected, enable local
playback during the probe. This still writes `zoom_sdk.output_wav`, but also
plays the received PCM through the default Windows speaker, or through
...
`zoom_sdk.meeting_number`, `zoom_sdk.password`, `zoom_sdk.sdk_root`,
`zoom_sdk.adapter_module = "modules.zoom_sdk_process_adapter"`, and
...
archiving gates, subscribes to mixed meeting raw audio, emits PCM to Python, and
then unsubscribes. If the run reports `SDKERR_NO_PERMISSION` or records zero
audio, the meeting host/account still needs to grant raw recording/raw data
access before the SDK path can feed Palabra.
...
with the JWT generated by Python, and `--sdk-join` creates the meeting service
and attempts to join. These have been verified locally with Zoom Meeting SDK
`7.1.0 (41845)`. A placeholder meeting number reached Zoom and failed with
result `8` (`MEETING_FAIL_MEETING_NOT_EXIST`), confirming the join callback path.
`--raw-audio-probe` is the first SDK input test for Palabra: it emits mixed
meeting PCM as process-adapter audio messages, which Python can record and play
locally with `--zoom-sdk-play-local`. The SDK auth JWT is not reused as `app_privilege_token`; set
`ZOOM_SDK_APP_PRIVILEGE_TOKEN` only when Zoom requires a separate app privilege
token. A real meeting join has reached `MEETING_STATUS_INMEETING`.
