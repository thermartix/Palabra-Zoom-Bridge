...
`--sdk-init` runs SDK initialization in a guarded child process, and
`--sdk-auth` initializes the SDK, waits for proxy detection, and authenticates
with the JWT generated by Python from `ZOOM_SDK_CLIENT_ID` and
`ZOOM_SDK_CLIENT_SECRET`. Both have been verified locally with Zoom Meeting SDK
`7.1.0 (41845)`. The next SDK step is joining a meeting after auth succeeds.
